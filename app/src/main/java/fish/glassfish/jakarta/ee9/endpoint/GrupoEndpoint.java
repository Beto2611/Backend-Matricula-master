...
import backend.matricula.controllers.GrupoController;
...
@ServerEndpoint(value = "/buscarGrupoPorId", decoders = {JsonObjectDecoder.class}, encoders = {JsonObjectEncoder.class})
public class GrupoEndpoint {
    ...
    public JSONObject messageAll(JSONObject message){
        JSONObject obj = new JSONObject();
        obj.put("action", "GETGRUPOSPORIDCURSO");
        obj.put("idCurso", message.getString("codigoCurso"));
        return obj;
    }
    ...
    public void onMessage(JSONObject message, Session session) throws IOException, EncodeException, SQLException {
        ...
        if (response != null) {
            ...
            this.broadcast(switch (response.optString("action")) {
                case "POSTGRUPO" -> messageAll(message);
                default -> null;
            });
        }
    }
